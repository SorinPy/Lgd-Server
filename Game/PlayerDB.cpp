			...
			if ( sSkillMgr->IsSkill(fields[1].GetUInt16()) )
			{
				...
				if ( type == 1 ) // TODO: Move code to CheckSkill!
				{
					...
					if (pSkillTree)
					{
						...
						if (pSkillTree->GetType() >= 1 || 
							(pSkillTree->GetSkill() != pSkillTree->GetRelatedSkill() && pSkillTree->GetRelatedSkill() != 0))
						{
							if (!this->ReplaceMasterSkill(pSkillTree->GetReplaceSkill(), pSkillTree->GetSkill(), fields[2].GetUInt8()))
							{
								this->MagicAdd(fields[1].GetUInt16(), fields[2].GetUInt8());
							}
							
						}else if (pSkillTree->GetSkill() == pSkillTree->GetRelatedSkill())
						{
							this->MagicAdd(fields[1].GetUInt16(), fields[2].GetUInt8());
						}
					}
...
//		else if ( type >= 2 ) //TODO: Load Majestic skills
//		{
