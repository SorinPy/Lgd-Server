...
void SkillMgr::LoadSkillTree()
{
	...
	pugi::xml_parse_result result = doc.load_file("Data/Skills/MasterSkillTree.xml");

	auto skilltree = doc.child("MasterSkillTree");

	if (result)
	{
		for (pugi::xml_node tool : skilltree)
		{
			std::cout << "Class:";

			for (pugi::xml_node child : tool.children())
			{
				auto type = child.attribute("Type").as_uint();
				for (auto skill : child)
				{
					auto attr = skill.attribute("Index");
					auto index = skill.attribute("Index").as_int();
					auto reqMinPoint = skill.attribute("ReqMinPoint").as_int();
					auto maxPoint = skill.attribute("MaxPoint").as_int();
					auto parentSkill1 = skill.attribute("ParrentSkill1").as_int();
					auto parentSkill2 = skill.attribute("ParentSkill2").as_int();
					auto magicNumber = skill.attribute("MagicNumber").as_int();
					auto valueType = skill.attribute("ValueType").as_int();
				}
			}
